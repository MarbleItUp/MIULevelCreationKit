...
namespace MIU.ProBuilder2
{
    public class QuadUV : EditorWindow
    {

        [MenuItem("Tools/Marble It Up/Quad UV")]
        ...
        [MenuItem("Tools/" + pb_Constant.PRODUCT_NAME + "/Actions/Quad UV - Selection", false, pb_Constant.MENU_ACTIONS + 20)]
        public static void ProcessQuadUVSelected()
        {
            List<pb_Object> selObjs = new List<pb_Object>();
            foreach(var v in Selection.transforms)
            {
                pb_Object p = v.GetComponent<pb_Object>();
                if (p != null)
                    selObjs.Add(p);
            }
            pb_Object[] sel = selObjs.ToArray();
            ProcessQuadUV(sel);
        }

        [MenuItem("Tools/" + pb_Constant.PRODUCT_NAME + "/Actions/Quad UV Strip - Selection", false, pb_Constant.MENU_ACTIONS + 20)]
        ...
        static bool ProcessQuadUV(pb_Object[] selected)
        {
            Undo.RecordObjects(selected, "Quad UV");
            var facesValid = 0;
            var facesInvalid = 0;
            var updatedFaces = new List<pb_Face>();
            for (int i = 0; i < selected.Length; i++)
            {
                var obj = selected[i];
                var uv = obj.uv;
                var faces = obj.SelectedFaces;
        ...
        static bool ProcessQuadUVStrip(pb_Object[] selected)
        {
